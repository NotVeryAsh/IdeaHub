...
@section('content')
    ...
    <form class="w-full max-w-xs mx-auto space-y-8" action="/auth/login?@if($email)redirect={{$redirect}}@endif" method="post" id="recaptcha-protected-form" data-sitekey="{{ config('services.recaptcha.key') }}" data-action="login">
        @csrf
        ...
        <div>
            ...
            <input @if($email) readonly @endif placeholder="Email or Username" id="username" type="text" name="identifier" maxlength="255" required value="{{ $email ?? old('identifier') }}" class="read-only:opacity-60 read-only:hover:cursor-not-allowed bg-gray-700 shadow-md appearance-none border rounded w-full py-2 px-3 leading-tight focus:outline-none @if($errors->has('identifier')) border-red-400 @else border-gray-500 @endif">
